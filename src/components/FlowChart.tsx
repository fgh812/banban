...
import { label, short, sums, won, wonShort } from '../lib/util'
  ...
  return (
    <>
      ...
      <div className="chartbox" ref={box}>
        <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="월별 수입·지출·남는 돈 추이"
          onMouseLeave={() => setTip(null)}
          onMouseMove={e => {
            const t = e.target as SVGElement
            if (!t.classList?.contains('hit')) return
            const r = box.current!.getBoundingClientRect()
            setTip({ i: +t.getAttribute('data-i')!, x: e.clientX - r.left + box.current!.scrollLeft, y: e.clientY - r.top })
          }}>
          {ticks.map(v => <g key={v}>
            <line x1={L} y1={y(v).toFixed(1)} x2={W - R} y2={y(v).toFixed(1)} stroke={v === 0 ? 'var(--line)' : 'var(--grid)'} />
            <text x={L - 9} y={(y(v) + 4).toFixed(1)} textAnchor="end" fontSize={11} fill="var(--muted)" fontFamily="IBM Plex Mono, monospace">{v === 0 ? '0' : wonShort(v)}</text>
          </g>)}
        ...
        {tip && (() => { const d = data[tip.i]; return (
          <div className="tip" style={{ left: Math.max(4, tip.x + 14), top: Math.max(4, tip.y - 10) }}>
            <b>{label(d.k)}</b>
